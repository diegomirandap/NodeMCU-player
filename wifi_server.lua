...
local ap_cfg={ssid="nodemcu12", pwd="reativos", save=false,
    staconnected_cb=function(cli) 
        print("Algu̩m conectou! MAC: " .. cli.MAC)
        abrehost('192.168.0.21')
    end,
    stadisconnected_cb=function(cli) print("Algu̩m desconectou! MAC: " .. cli.MAC) end
    }
...
function abrehost(ipsta)
    local udp_client = net.createUDPSocket()

    udp_client:listen(1234) 
    print("conectou")

    udp_client:on("receive", function(socket, message, port, ip)
        --if message.sub(1,6)=="INICIO" then
